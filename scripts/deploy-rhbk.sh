...
if [ -n "${REALM_NAME:-}" ]; then
    REALM_NAME_SET_BY_ENV=1
else
    REALM_NAME=cost-management
fi
...
create_or_update_user() {
    ...
    local org_id
    org_id=$(normalize_org_id "$(echo "$user_json" | jq -r '.orgId // "1234567"')")
...
display_summary() {
    ...
    while [ $u -lt "$u_count" ]; do
        ...
        u_org=$(normalize_org_id "$(echo "$USERS_JSON" | jq -r ".[$u].orgId // \"1234567\"")")
...
# If a values file was provided, read the realm name from jwtAuth.keycloak.realm
# (unless REALM_NAME was explicitly set via environment variable).
if [ -n "$VALUES_FILE" ] && [ -z "${REALM_NAME_SET_BY_ENV:-}" ]; then
    _values_realm=$(python3 -c "
import sys, yaml
with open(sys.argv[1]) as f:
    vals = yaml.safe_load(f)
print(vals.get('jwtAuth', {}).get('keycloak', {}).get('realm', ''))
" "$VALUES_FILE" 2>/dev/null || true)
    if [ -n "$_values_realm" ]; then
        REALM_NAME="$_values_realm"
    fi
fi
