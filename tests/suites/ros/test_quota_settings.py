"""E2E tests for ROS ResourceQuota settings API.

Endpoints:
  GET/PUT/DELETE /api/cost-management/v1/recommendations/openshift/settings/quota
"""
...
def _quota_settings_url(ros_api_url: str) -> str:
    return (
        f"{ros_api_url.rstrip('/')}/cost-management/v1/"
        "recommendations/openshift/settings/quota"
    )
...
def _skip_if_plugin_disabled(resp: requests.Response) -> None:
    if resp.status_code == 404:
        pytest.skip(
            "Quota recommendations plugin not enabled (404 on /settings/quota)"
        )
...
def _get_settings(
    ...
    return session.get(
        _quota_settings_url(ros_api_url),
        headers=auth,
        timeout=30,
    )
...
@pytest.fixture
def quota_settings_auth(keycloak_config, cluster_config, http_session):
...
class TestQuotaSettingsE2E:
    """ResourceQuota threshold settings CRUD against a deployed cluster."""

    def test_quota_settings_get_defaults(
        self,
        ros_api_url: str,
        quota_settings_auth: dict,
        http_session: requests.Session,
    ):
        resp = _get_settings(http_session, ros_api_url, quota_settings_auth)
        _skip_if_plugin_disabled(resp)
        assert resp.status_code == 200, resp.text
        body = resp.json()
        for field, expected in _DEFAULTS.items():
            assert field in body, f"missing {field!r} in quota settings"
            assert body[field] == expected
        assert isinstance(body.get("locked_fields"), list)

    def test_quota_settings_put_persists(
        self,
        ros_api_url: str,
        quota_settings_auth: dict,
        http_session: requests.Session,
        ...
        locked_fields = _fetch_locked_fields(
            http_session, ros_api_url, quota_settings_auth
        )
    ...
    def test_quota_settings_delete_resets_defaults(
        self,
        ros_api_url: str,
        quota_settings_auth: dict,
        http_session: requests.Session,
    ...
    def test_quota_settings_put_validation_headroom_over_100(
        self,
        ros_api_url: str,
        quota_settings_auth: dict,
        http_session: requests.Session,
    ...
    def test_quota_settings_put_validation_medium_gte_high(
        self,
        ros_api_url: str,
        quota_settings_auth: dict,
        http_session: requests.Session,
