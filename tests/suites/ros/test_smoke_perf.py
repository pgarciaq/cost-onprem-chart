...
def _measure_best_elapsed(
    request_fn: Callable[[], requests.Response],
    *,
    attempts: int = PERF_RETRY_ATTEMPTS,
) -> tuple[float, requests.Response | None, list[float]]:
    """Run request_fn up to attempts times and return best elapsed time."""
    timings: list[float] = []
    ...
    return _best_elapsed_seconds(timings), last_response, timings
...
class TestROSSmokePerformance:
    ...
    def test_container_list_responds_within_threshold(
        ...
        elapsed, last_response, timings = _measure_best_elapsed(
    ...
    def test_ros_status_responds_within_threshold(
        ...
        """ROS status endpoint should respond within 2 seconds (best of 2 attempts)."""
        auth_header = get_fresh_auth_header(keycloak_config, http_session)
        if not auth_header:
            pytest.skip("Could not obtain fresh JWT token")

        url = f"{ros_api_url.rstrip('/')}/cost-management/v1/status/"
        elapsed, response, timings = _measure_best_elapsed(
            lambda: http_session.get(
                url,
                headers=auth_header,
        ...
        assert elapsed < STATUS_MAX_SECONDS, (
            f"Status best-of-{PERF_RETRY_ATTEMPTS} took {elapsed:.2f}s "
            f"(threshold {STATUS_MAX_SECONDS}s; all attempts: {timings})"
        )
