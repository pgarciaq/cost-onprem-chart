# Development & Testing Documentation

Testing guides, validation procedures, and development resources for Cost Management On-Premise.

## Documents

| Document | Description |
|----------|-------------|
| **[Performance Testing Plan](../performance/performance-testing-plan.md)** | FLPATH-4036 performance testing strategy and implementation |
| **[Test Data Setup](test-data-setup.md)** | Automatic session seeding and scenario-based manual setup |
| **[OCP Dev Setup with S4](ocp-dev-setup-s4.md)** | Set up a dev environment on OCP using S4 instead of ODF |
| **[UI OAuth Testing](ui-oauth-testing.md)** | Guide for testing UI OAuth flow with Keycloak |
| **[IQE Testing Setup](iqe-testing-setup.md)** | Prerequisites and setup for IQE integration tests |
| **[Skipped IQE Tests](skipped-iqe-tests.md)** | Test profiles, skip groups, markers, and known blockers |
| **[Test Impact Map](test-impact-map.md)** | How the automated test recommendation system maps components to profiles |
| **[Test Coverage Analysis](test-coverage-analysis.md)** | Chart pytest vs IQE test coverage comparison |

## Additional Resources

For test scripts and automation, see:
- **[../scripts/README.md](../../scripts/README.md)** - Test execution scripts and E2E validation
- **[../tests/README.md](../../tests/README.md)** - PyTest suite documentation

[← Back to Documentation Index](../README.md)
